...
class BaseService(object):
    ...
    def log(self, log_type, text):
        print time.strftime('%Y_%m_%d %H_%M_%S')+" "+log_type+": "+text
        logger.info(text)
    ...
    def movetofinish(self):
        ...
        os.rename(os.path.join(self.path_incoming, self.filename), os.path.join(self.path_finished, time.strftime('%Y_%m_%d')+"_"+self.filename))
        #os.rename(os.path.join(self.path_incoming, self.filename), os.path.join(self.path_finished, self.filename))
    ...
    def insert(self, obj):
        ...
        obj['hash'] = zhash
        done = self.mongo_collection.update({"hash": zhash}, obj, True)
